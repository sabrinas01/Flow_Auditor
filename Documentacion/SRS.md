...
**Versión:** 4.5 (Test de paridad local/CI para credenciales)  
...
| **v4.5** *(Actual)* | Agosto 2026 | Sabrina Sanso | **Test de paridad local/CI (`SRS-FR-M2-201`, `SRS-FR-M2-203`):**<br>- 2 tests nuevos en `tests/test_paridad_local_ci.py`: verifican que la ruta CI (Secrets ya en el entorno) y la ruta local (`.env` cargado vía `cargar_env_local()`) resuelvan exactamente la misma credencial final, tanto con los nombres primarios (`NOTION_API_KEY`) como con los alternativos (`NOTION_TOKEN`/`NOTION_DATABASE_ID`).<br>- Se descarta explícitamente crear `.env.example`: no aporta valor suficiente frente al costo de mantenerlo sincronizado, y las variables ya están documentadas en `Documentacion/configuration.md`. |
