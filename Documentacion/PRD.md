...
| **v4.13** *(Actual)* | Agosto 2026 | **Recordatorios Varios: acotar a "Por hacer" y ordenar por fecha de creación (pedido explícito de Sabrina, tras reportar que seguía mostrando estados "En curso" y "Completado"):** `_es_estado_pendiente()` se reemplaza por `_es_estado_por_hacer()`, una whitelist explícita (solo "Sin empezar" y "⏳ Pospuesta") en vez de una lista de exclusión — v4.12 excluía únicamente el grupo Completado y dejaba pasar "En ejecución"/"En espera" (grupo En curso). Además, dentro de cada bloque (Ayer/Hoy/Mañana) los ítems ahora quedan ordenados por fecha de creación ascendente. Ver `SRS.md` v4.13 — `SRS-FR-M4-402` reescrito. |
