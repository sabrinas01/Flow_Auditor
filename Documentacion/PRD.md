...
* **Modelo de datos (v4.12):** solo se muestran ítems **pendientes** (Estado distinto de "Hecha", "Hecha por otra persona", "⏭️ No necesaria" y "❌ Fallida / Vencida"), clasificados por las mismas tres ventanas cronológicas que Recordatorios Diarios (Ayer/Hoy/Mañana, GMT-3) — pero a partir de la **fecha de creación** del recordatorio en Notion, no de su propiedad `Fecha` (que sigue mostrándose como metadato de cada ítem). A diferencia de Recordatorios Diarios, cada bloque **no** agrega por conteo de estados: renderiza una **lista de ítems individuales**, cada uno con Nombre, Estado, Prioridad, Área, Periodo y Fecha. (v4.10 mostraba todos los ítems sin filtrar por fecha; v4.11 filtraba por la propiedad `Fecha` sin filtrar por estado — v4.12 agrupa por fecha de creación y limita a pendientes, por pedido explícito de Sabrina.)
