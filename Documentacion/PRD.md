...
La propiedad **Consistencia** es una fórmula de Notion, definida igual en Recordatorios Diarios y Recordatorios Varios: `if(prop("Estado") == "Hecha" or prop("Estado") == "Hecha por otra persona" or prop("Estado") == "No necesaria", 1, 0)`.
...
| **v4.20** | Septiembre 2026 | **Reordenar menú hamburguesa por prioridad de uso diario (HU Notion #8, pedido explícito de Sabrina, prioridad Alta):** el sidebar de las 4 páginas pasa del orden Inicio/Recordatorios diarios/Recordatorios varios/Agenda personal al orden fijo Inicio/Agenda personal/Recordatorios diarios/Recordatorios varios (nuevo §5.4). Cambio puramente de marcado — se reordenaron los `<a>` del `<nav>` sin tocar clases ni lógica de resaltado del ítem activo. Sin cambios de backend. Ver `SRS.md` v4.20 — nuevo `SRS-FR-M3-312`. |
| **v4.21** | Septiembre 2026 | **Test de regresión — publicación completa de frontends en gh-pages (HU Notion #10, cierra `SRS-FR-M2-206`):** nuevo `tests/test_publicacion_gh_pages.py`, que parsea estáticamente `notion_sync.yml` y `extract_and_audit.py` y falla si el `git add` del step "Publish" no incluye todos los frontends que el script efectivamente escribe (el bug real de v4.8: un frontend nuevo puede quedar congelado con datos viejos en `gh-pages` sin que nada lo detecte). Sin cambios de comportamiento del pipeline, solo cobertura de test nueva. Ver `SRS.md` v4.21. |
| **v4.22** | Septiembre 2026 | **Test de regresión — rebote de navegación inicio.html/index.html (HU Notion #9, cierra `SRS-FR-M3-309`):** nuevo `tests/unit/navegacion_landing.test.js` (Jest + jsdom), que extrae el script real de `nfa_landed` de cada archivo y lo ejecuta en una ventana jsdom aislada, cubriendo tanto que `inicio.html` setee la marca sin redirigir como que `index.html` no rebote si la marca ya existe. Se evaluó Playwright/Puppeteer y se descartó a favor de `jsdom` (ya transitivo vía Jest, sin sumar descarga de navegadores ni tiempo de CI). Sin cambios de comportamiento del sitio, solo cobertura de test nueva. Ver `SRS.md` v4.22. |
| **v4.23** *(Actual)* | Septiembre 2026 | **Tests JS dedicados — render de Recordatorios Varios y Eventos de Agenda Personal (HU Notion Épica 2 #11, cierra `SRS-FR-M4-404`/`SRS-FR-M5-505`):** `renderizarRecordatoriosVarios()` y `formatHora()`/`renderizarAgendaEventos()` (antes inline, sin test) se extraen a `src/utils/dom_render.js`, mismo patrón que en v4.4, y se cubren con tests nuevos en `tests/unit/dom_render.test.js`. Ambas páginas ya cargaban ese archivo, así que es un cambio puramente interno — sin diferencia de comportamiento visible. Ver `SRS.md` v4.23. |
