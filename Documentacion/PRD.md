...
* **Backend Extractor (Python 3.10):** Daemon invisible (Windows) o asíncrono (GitHub Actions). Consulta **cuatro bases de Notion independientes** (Recordatorios Diarios, Recordatorios Varios, Agenda Personal - Eventos y Agenda Personal - Recordatorios) y realiza inyección dinámica de datos (JSON) sobre los archivos `index.html`, `recordatorios-varios.html` y `agenda-personal.html` respectivamente, en el mismo ciclo horario.
...
* Módulo alimentado por **dos bases de Notion propias e independientes**, ninguna compartida con los módulos anteriores: `NOTION_DB_EVENTOS` ("Agenda Personal - Eventos": Nombre, Fecha con hora y rango inicio/fin, Lugar) y `NOTION_DB_RECORDATORIOSUNICOS` ("Agenda Personal - Recordatorios": Nombre, Estado). Ambas opcionales — su ausencia o fallo no afecta a ningún otro módulo.
* **Eventos:** clasificados por su propia fecha programada (no por fecha de creación) en dos ventanas únicamente — **Hoy** y **Mañana** — sin bloque Ayer: un evento pasado no aporta a una agenda, mismo criterio de recorte que Recordatorios Diarios aplicó a "Mañana" (§5.1). Dentro de cada bloque se ordenan ascendentemente por hora de inicio.
* **Recordatorios:** mismo modelo que Recordatorios Varios (§5.2) — solo ítems del grupo "Por hacer" ("Sin empezar" / "⏳ Pospuesta"), clasificados en las tres ventanas cronológicas (Ayer/Hoy/Mañana) según su fecha de creación en Notion, contra su propia base independiente.
* **Secciones:** * I: Eventos de hoy / Eventos de mañana (2 cards colapsables).
    * II: Recordatorios de ayer / hoy / mañana (3 cards colapsables, igual patrón visual que §5.2).
    * III: CTA "Ver en Notion" (uno por cada base) + Panel de Diagnóstico Técnico (footer, igual a §5.1-V).
* **Solo lectura:** ningún elemento de la interfaz escribe o modifica datos en Notion (§7) — los recordatorios se muestran con el mismo estilo visual (`status-row`/pill) que Recordatorios Varios, sin checkbox interactivo.
...
* **Aislamiento de fallos:** ambas bases se sincronizan en su propio paso aislado y no fatal, después de Recordatorios Varios — un fallo en cualquiera de las dos (o en ambas) nunca afecta a `index.html` ni a `recordatorios-varios.html`, y un fallo en una no afecta a la otra.
