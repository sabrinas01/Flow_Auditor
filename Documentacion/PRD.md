...
| **v4.4** *(Actual)* | Agosto 2026 | Se implementa el monitoreo de RAM dentro del script (antes solo existía como test externo); se implementa por primera vez la jerarquía de estados del §4.2 (nunca estuvo codificada, solo documentada); se agregan tests de renderizado DOM. Ver `SRS.md` v4.4 — `SRS-FR-M3-308` nuevo. |
